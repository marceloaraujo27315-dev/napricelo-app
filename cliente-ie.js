(function(){
  ...
  if(form&&!form.elements.inscricao_estadual){const doc=form.querySelector('input[name="documento"]')?.closest('label');if(doc){const lab=document.createElement('label');lab.innerHTML='Inscrição Estadual<input name="inscricao_estadual" placeholder="PJ, produtor rural ou pessoa física com IE">';doc.insertAdjacentElement('afterend',lab);}}
  if(typeof salvarRegistroCadastro==='function'){const baseSalvar=salvarRegistroCadastro;salvarRegistroCadastro=async function(tabela,payload){if(tabela==='clientes'){payload={...payload,inscricao_estadual:document.getElementById('clienteForm')?.elements.inscricao_estadual?.value||null};}return baseSalvar(tabela,payload);};}
