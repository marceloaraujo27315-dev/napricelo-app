...
async function uploadFotoManutencao(file,codigo,etapa){
  ...
  const r=await fetch(`${PHOTO_SUPABASE_URL}/storage/v1/object/${PHOTO_BUCKET}/${path}`,{
    method:"POST",
    headers:{
      apikey:PHOTO_SUPABASE_KEY,
      Authorization:`Bearer ${PHOTO_SUPABASE_KEY}`,
...
async function salvarManutencaoComFotos(o,eq,fotos){
  ...
  const payload={
    equipamento_id:eq.id||null,tipo:o.tipo||null,codigo:eq.codigo||null,cliente:eq.cliente||null,
    unidade:eq.unidade||null,municipio:eq.municipio||null,localizacao:eq.localizacao||eq.local||null,
    data:o.data||null,tecnico:o.tecnico||null,checks:o.checks||[],campos,observacoes:o.observacoes||null,
    foto_antes:fotos.antes||null,foto_durante:fotos.durante||null,foto_depois:fotos.depois||null
  };
  const r=await fetch(`${PHOTO_SUPABASE_URL}/rest/v1/manutencoes`,{
    method:"POST",
    headers:{apikey:PHOTO_SUPABASE_KEY,Authorization:`Bearer ${PHOTO_SUPABASE_KEY}`,"Content-Type":"application/json",Prefer:"return=representation"},
...
function prepararFotosManutencao(){
  document.querySelectorAll(".manutForm").forEach(f=>{
    ...
    f.addEventListener("submit",async ev=>{
      ...
      const sel=f.querySelector(".equipSelect");
      const box=f.querySelector(".equipAuto");
      const idx=Number(sel.value), eq=eqs()[idx];
      if(!eq) return alert("Selecione um equipamento.");
      const fd=new FormData(f);
      const o={tipo:f.dataset.tipo,equip_index:idx,codigo:eq.codigo,cliente:eq.cliente,unidade:eq.unidade,municipio:eq.municipio,local:eq.localizacao||eq.local,criado:new Date().toLocaleString("pt-BR")};
      ...
      try{
        ...
        const inputs=[...f.querySelectorAll('input[type="file"]')];
        const fotos={};
        for(const inp of inputs){
          const file=inp.files?.[0];
          if(file) fotos[inp.dataset.etapa]=await uploadFotoManutencao(file,eq.codigo,inp.dataset.etapa);
        }
        const salvo=await salvarManutencaoComFotos(o,eq,fotos);
        const a=db.get("manut");a.push({...o,...salvo});db.set("manut",a);
        alert(Object.keys(fotos).length?"Manutenção e fotos salvas na nuvem.":"Manutenção salva na nuvem.");
        f.reset();box.textContent="Selecione o equipamento.";showPage("home");
      }catch(err){
        console.error(err);
        alert("Não foi possível salvar a manutenção/fotos na nuvem. Verifique a internet, o tamanho e o formato das imagens e tente novamente.");
      }finally{
        if(btn){btn.disabled=false;btn.textContent="Salvar manutenção";}
      }
    },true);
  });
}
