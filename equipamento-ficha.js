...
function prepararSelecaoEquip(page,id){
  fecharFichaEquipamento();fecharFichaCliente();showPage(page);refreshEquipSelectors();
  const idx=acharIndiceEquip(id);if(idx<0)return alert("Equipamento não encontrado na lista atual.");
  setTimeout(()=>{
    const sel=page==="analise"?document.getElementById("analiseEquip"):document.querySelector(`#${page} .equipSelect`);
    if(sel){sel.value=String(idx);sel.dispatchEvent(new Event("change",{bubbles:true}));sel.scrollIntoView({behavior:"smooth",block:"center"});}
  },100);
}
...
window.cartaoEquipCliente=function(e,manut,analises){
  const ms=manut.filter(x=>Number(x.equipamento_id)===Number(e.id));const as=analises.filter(x=>Number(x.equipamento_id)===Number(e.id));const ultimaM=ms[0],ultimaA=as[0];
